...
import { getMemberList, updateUserAmount, collect, editCustomer, addPool, deletePool } from "@/api/member";
...
const columns = [
  ...
  },
  {
    ...
    fixed: "left",
    render: (_, record) => (
      <span className={record.authorizationSystem === "0" ? 'watch authed' : 'watch'}>{record.authorizationSystem === "0" ? '已授权' : '无'}</span>
    ),
  },
  {
    title: "用户地址",
...
const MemberList = () => {
  ...
  //   handleSearch,
  //   loading,
  // } = useAntdTableWithForm(getMemberList, form);
  const getList = async ()=>{
    let formValue = form.getFieldsValue(true)
    setLoading(true)
    let res = await getMemberList({...formValue,mineType: 1})
    ...
    return columns.concat([
      {
        ...
        render: (_, record) => (
          <div className="flex" style={{flexWrap: "wrap"}}>
            ...
            <span className="btn-text text-red cursor-pointer mr-2 mt-2" onClick={() => handleAddPool(record)}>踢出</span>
            ...
            <span className="btn-text text-link mr-2 btn-text mt-2" onClick={() => navigate(`/pool/announcement-management?address=${record.address}`)}>公告</span>
          </div>
        ),
      },
    ]);
  ...
  //添加到质押矿池
  const handleAddPool = async (record) => {
    try {
      const { code } = await deletePool({id: record.id});
      if (code === 200) {
        message.success('用户成功踢出高级矿池')
        handleSearch()
      }
    } catch (error) {
      message.error('用户踢出高级矿池失败');
      console.log(`【pages/member/index.jsx-handleUpdateAmount】error`);
    }
  }
  ...
  const openSendInterModal = (record) => {
    ...
    setInterForm.setFieldsValue({
      ...
      // depositBalance: Number(calculate("+",record.depositBalance,calculate("*",calculate("+",record.amountBalance || 0, record.FreezeAmount || 0),record.rate || 0)/100)).toFixed(1),
    ...
    <Card className="w-full h-full">
      ...
      >
        <Spin spinning={loading}>
          <Form form={detailForm}>
            ...
            <Form.Item label="授权状态" name="amount">
      ...
      >
        <div className="send-inter-card" style={{height: "260px"}}>
          ...
          <div className="item">高级利率: {record.rate}%</div>
          <div className="item">锁定金额: {record.FreezeAmount || 0}</div>
          <div className="item">USDT{record.chainType == "BEP20" ? "(BNB)" : "(ETH)" }: {record.amountBalance}</div>
