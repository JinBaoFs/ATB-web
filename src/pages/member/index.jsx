...
const MemberList = () => {
  ...
  const getList = async ()=>{
    ...
    setLoading(true)
    let res = await getMemberList({...formValue,mineType: 0,})
    ...
    return columns.concat([
      {
        ...
        render: (_, record) => (
          <div className="flex" style={{flexWrap: "wrap"}}>
            ...
            <span className="btn-text text-red cursor-pointer mr-2 mt-2" onClick={() => openTips(record)}>高级</span>
  ...
  const handleAddPool = async () => {
    ...
    if(record.pledgeMines == '0'){
      message.error(`用户已添加到质押矿池`);
      return;
    }
    if (record?.address) {
      options.day = 30,
    ...
    <Card className="w-full h-full">
      ...
      >
        <div className="send-inter-card" style={{height: "260px"}}>
          ...
          <div className="item">普通利率: {record.rate}%</div>
        ...
        <div className="send-form-box">
          <div className="send-form-item">
            <Form form={setInterForm}  labelCol={{span:6}} wrapperCol={{span: 16}} disabled>
              <div style={{paddingLeft: "10px",marginBottom: "15px", fontWeight: "bold"}}>派发前收益</div>
              <Form.Item label='收益总额' name='sumIncome_tow'>
                <Input type="number" 
                  placeholder='请输入收益总额'
                />
              </Form.Item>
          ...
          <div className="send-form-item">
            <Form form={setInterForm}  labelCol={{span:6}} wrapperCol={{span: 16}}>
              <div style={{paddingLeft: "10px",marginBottom: "15px", fontWeight: "bold"}}>派发后收益</div>
              <Form.Item label='收益总额' name='sumIncome' rules={[{ required: true, message: '请输入收益总额'},]}>
